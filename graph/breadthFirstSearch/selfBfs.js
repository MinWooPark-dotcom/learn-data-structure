...
// graph: 정점(Vertex)간의 관계를 표현하는 조직도
class Graph {
    ...
    bfs(startingNode)
    {
        //  접근한 정점의 인접 리스트 중 접근하지 않은 값을 큐에 넣어 순서대로 방문한다.
        const visited = {};

        const queue = new Queue();
        // while문 돌리기 위해 startingNode 방문 처리 및 enqueue
        visited[startingNode] = true;
        queue.enqueue(startingNode);

        while(!queue.isEmpty()) {
            const getQueueData = queue.dequeue();
            console.log('getQueueData', getQueueData) // 방문한 vertex
            const getAdList = this.AdjList.get(getQueueData);
            // console.log('getAdList', getAdList)
            // bfs는 현재 정점의 인접 리스트를 모두 순회하고 현재 인접리스트의 인접리스트로 넘어감. 그래서 현재 인접 리스트 중 방문하지 않은 정점은 모두 enqueue
            for (const i in getAdList) {
                const neigh = getAdList[i];
                if (!visited[neigh]) {
                    visited[neigh] = true;
                    queue.enqueue(neigh);
                }
