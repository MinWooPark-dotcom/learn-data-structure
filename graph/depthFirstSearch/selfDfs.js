// 깊이 우선탐색 DFS는 갈 수 있는 만큼 최대한 깊이 가고, 더 이상 갈 곳이 없다면 이전 정점으로 돌아가는 방식으로 그래프를 순회하는 방식입니다.
// 간단히 재귀호출을 사용하여 구현하거나 스택을 사용하여 구현합니다.
...
class Graph {
    ...
    // 사용하는 경우: 모든 노드를 방문 하고자 하는 경우에 이 방법을 선택한다.
     정점의 인접 리스트의 첫 번쨰 정점을 방문하고, 그 정점의 인접한 리스트의 첫 번째 정점에 방문하고 인접리스트가 없으면 다시 돌아와 다음 정점으로 접근 **/
    // 출처: https://gmlwjd9405.github.io/2018/08/14/algorithm-dfs.html
    // Main DFS method
    dfs(startingNode)
    {
        const visited = {};
        this.DFSUtil(startingNode, visited) // 재귀 시 visited가 초기화 되지 않도록 DFSUtil을 만들어 로직을 나눔.
    }
...
// all the adjacent vertex of the vertex with which it is called
    ...
    {
        visited[vert] = true;
        console.log(vert) // 방문한 정점

        const adList = this.AdjList.get(vert); // 정점의 인접 리스트

        for (const i in adList) {
            if (!visited[adList[i]]) {
                this.DFSUtil(adList[i], visited);
            }
        }
    }
}
